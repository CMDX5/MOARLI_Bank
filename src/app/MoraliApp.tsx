...
/** Strip HTML/XML tags and trim whitespace */
const sanitizeInput = (value: string, maxLen = 200): string =>
  String(value || "")
...
/** Sanitize a numeric input — returns NaN if invalid */
...
const firebaseAuthMessage = (error: unknown) => {
...
const getIdentitySeed = (email?: string | null, uid?: string | null) => {
  const normalizedEmail = (email || "").trim().toLowerCase();
  return normalizedEmail || uid || "morali-default";
};

const generateMoraliIdentity = (seed?: string) => {
  const source = seed && seed.trim() ? seed.trim() : "morali-default";
  let hash = 0;
  for (let i = 0; i < source.length; i += 1) {
    // Fowler-Noll-Vo inspired: prime multiplier 1099511628211 mod 900M → 9 digits
    hash = ((hash * 1099511628) + source.charCodeAt(i)) % 900000000;
  }
  // Ensure at least 1 to avoid MORALI00000
...
const getIdentityCacheKey = (uid: string) => `morali_identity_${uid}`;

const getCachedIdentityForUid = (uid: string) => {
...
const cacheIdentityForUid = (uid: string, identity: { id: string; rib: string }) => {
...
function formatCurrency(value: number) {
  // Always positive — absolute value, never shows a sign
  return new Intl.NumberFormat("fr-FR").format(Math.abs(value));
}

function formatAmount(value: number, type: "credit" | "debit") {
  // Sign is controlled solely by type, value is always positive via formatCurrency
  const sign = type === "credit" ? "+" : "-";
  return `${sign} FCFA ${formatCurrency(value)}`;
}

// Format for stat cards: "+ 260 000" or "- 110 000" with proper spacing
function formatStat(value: number, type: "credit" | "debit") {
...
function App() {
  ...
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        if (transferOpen) { resetTransferFlow(); return; }
    ...
    try {
      ...
      if (data.app === "MoraliBank" && data.userId) {
        ...
        setTimeout(() => {
          ...
          setTransferRecipientQuery(data.userId);
        }, 1500);
      } else {
  ...
  const openTransferModal = () => {
    ...
    window.setTimeout(() => transferInputRef.current?.focus(), 60);
  };
  ...
  // Debounced auto-search for recipient
  ...
  // handleTransferRecipientQuery is defined above with debounced auto-search
  ...
  const handleTransferPad = (value: string) => {
    ...
    if (value === "max") {
      setTransferAmountInput(String(dashboardData.balance));
      return;
    }
  ...
  const executeTransfer = async () => {
    ...
    const userBalance = firestoreBalance !== null ? firestoreBalance : dashboardData.balance;
    if (amount > userBalance) { showToast("Solde insuffisant pour ce virement"); setTransferProcessing(true); setTimeout(() => setTransferProcessing(false), 100); return; }
    ...
    try {
      if (authUid && transferRecipient) {
        ...
        if (recipientUid === authUid) { showToast("Transfert vers vous-même impossible"); setTransferProcessing(false); return; }
      ...
      // Calculate post-transfer balance locally (before Firestore listener updates)
      const currentBal = firestoreBalance !== null ? firestoreBalance : dashboardData.balance;
      setTransferPostBalance(currentBal - Number(transferAmountInput || 0));
  ...
  const startTransferPin = async () => {
    ...
    setTimeout(() => {
      setTransferSliding(false);
      setTransferSlideProgress(0);
      // Always require PIN — check server first if hash not cached locally
      setTransferPinOpen(true);
      setTransferStage("pin");
    }, 320);
  };

  // Check if user has a PIN set on server (for PIN-less local state)
  const checkServerPin = async (): Promise<boolean> => {
    try {
      const user = firebaseAuth.currentUser;
  ...
  }, [transferOpen, transferSlideProgress]);
    ...
    <>
      ...
      <div className="stage">
        <div className="app-viewport">
          <div className={`app-screen ${screen === "auth" ? "active" : ""}`}>
            <div className="auth-scroll">
                ...
                <div className={`auth-panel ${authTab === "login" ? "active" : ""}`}>
                  ...
                  {/* Google Auth button hidden — feature unavailable */}
                ...
                <div className={`auth-panel ${authTab === "register" ? "active" : ""}`}>
                  ...
                  {/* ── PIN Setup Step (after OTP, before success) ── */}
                  {showPinSetup && (
                    <div className="pin-setup-wrap">
                      ...
                      <div className="pin-dots-row">
                        {[0, 1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className={`pin-dot ${(regPinStep === "create" ? regPinDraft : regPinConfirm).length > i ? "filled" : ""}`}
                          />
                        ))}
                      </div>

                      {/* Visible input to trigger native mobile keyboard */}
                      <input
                        type="password"
                        inputMode="numeric"
                        maxLength={4}
                        ...
                        onChange={(e) => {
                          ...
                          if (regPinStep === "create") {
                            setRegPinDraft(val);
                          } else {
                            setRegPinConfirm(val);
                          }
                        }}
                        placeholder="••••"
                        autoFocus
                      ...
                      {/* Continue button on create step */}
                      {regPinStep === "create" && regPinDraft.length === 4 && (
                        <button
                          className="btn-primary pin-confirm-btn"
                          onClick={() => { setRegPinStep("confirm"); setRegPinConfirm(""); }}
                      ...
                      {regPinStep === "confirm" && (
                        <button className="pin-back-link" onClick={handleRegPinBack}>
                      ...
                      {regPinStep === "confirm" && regPinConfirm.length === 4 && (
                        <button
                          className="btn-primary pin-confirm-btn"
                          onClick={handleRegPinSave}
                          disabled={regPinSaving}
                          style={{ width: "100%", marginTop: 20 }}
                        >
                    ...
                    <div className="transfer-modal" onClick={(event) => event.stopPropagation()}>
                        ...
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          {transferStage !== "search" && transferStage !== "processing" && transferStage !== "error" && (
                            <button className="contact-modal-close" onClick={() => {
                              ...
                              else if (transferStage === "success") { setTransferOpen(false); resetTransferFlow(); setScreen("dashboard"); }
                      ...
                      {transferStage === "success" && (
                        <div className="transaction-success-wrap" style={{padding:"24px 20px 18px",gap:14}}>
                            ...
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                              <div>
                                ...
                                <div style={{fontSize:20,fontWeight:900,color:"#fff",fontFamily:"'Montserrat',sans-serif",letterSpacing:"-0.5px"}}>
                                  {formatCurrency(transferPostBalance !== null ? transferPostBalance : (firestoreBalance !== null ? firestoreBalance : dashboardData.balance))} <span style={{fontSize:12,fontWeight:600,color:"#60a5fa"}}>FCFA</span>
                                </div>
                              </div>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                            <button className="transfer-home-btn" onClick={() => { setTransferOpen(false); resetTransferFlow(); setScreen("dashboard"); showToast("Virement terminé"); }}>
                              Accueil
                            </button>
                      ...
                      {transferStage === "error" && (
                        <div className="transaction-success-wrap" style={{padding:"32px 20px 20px",gap:18}}>
                          ...
                          <button className="transfer-home-btn" onClick={() => { setTransferOpen(false); resetTransferFlow(); setScreen("dashboard"); }}>
                            Retour à l'accueil
                          </button>
                        </div>
                      )}
          ...
          <div className={`app-screen ${screen === "cards" ? "active" : ""}`}>
            <div className="content-scrollable nav-safe">
                ...
                <div className="cards-duo">
                      ...
                      >
                        ...
                        <div className="vc-content">
                          ...
                          <div className="vc-number" style={{ cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); setEssentialCardRevealed((v) => !v); }}>
                            {essentialCardRevealed ? dashboardData.cardNumber : maskCardNumber(dashboardData.cardNumber)}
                          </div>
                          ...
                          <div className="vc-bottom-row">
                            ...
                            <div className="vc-ccv-wrap">
                              ...
                              <div className="vc-ccv-v">{essentialCardRevealed ? dashboardData.cardCcv : "•••"}</div>
                            </div>
                            <div className="vc-exp-visa-wrap">
                              <div className="vc-exp-wrap">
                                <div className="vc-exp">
                                  <div className="vc-exp-l">Expire</div>
                                  <div className="vc-exp-v">{dashboardData.cardExp}</div>
                                </div>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="cards-duo-card">
                      ...
                      >
                        ...
                        <div className="vc-content">
                          ...
                          <div className="vc-number" style={{ cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); setBlackCardRevealed((v) => !v); }}>
                            {blackCardRevealed ? dashboardData.blackCardNumber : maskCardNumber(dashboardData.blackCardNumber)}
                          </div>
                          ...
                          <div className="vc-bottom-row">
                            ...
                            <div className="vc-ccv-wrap">
                              <div className="vc-ccv-l">CCV</div>
                              <div className="vc-ccv-v">{blackCardRevealed ? dashboardData.blackCardCcv : "•••"}</div>
                            </div>
                            <div className="vc-exp-visa-wrap">
                              <div className="vc-exp-wrap">
                                <div className="vc-exp">
                                  <div className="vc-exp-l">Expire</div>
                                  <div className="vc-exp-v">{dashboardData.blackCardExp}</div>
                                </div>
                              </div>
                            </div>
                          </div>
                    ...
                    <div className="cards-duo-info">
                      ...
                      <div className="cards-duo-stat">
                        <button onClick={() => setHistoryModalOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                ...
                <div className="card-actions-grid">
                  {cardActions.map((item) => (
                    <button
                      ...
                      onClick={() => item.label === "Geler la carte" ? openManageCardModal() : item.label === "Code PIN" ? openPinModal() : item.label === "Limites" ? openCardLimitsModal() : item.label === "Nouvelle" ? openVirtualCardModal() : showToast(item.label)}
                    >
          ...
          <div className={`app-screen ${screen === "profile" ? "active" : ""}`}>
            <div className="content-scrollable nav-safe">
              <div className="profile-screen">
                <div className="profile-top">
                  ...
                  <div style={{ textAlign: "center" }}>
                    <div className="profile-name">{dashboardData.holder}</div>
                    <div className="profile-id">ID: {bankingIdentity.id}</div>
                ...
                {profileGroups.map((group) => (
                  <div key={group.title} className="profile-group">
                    ...
                    {group.items.map((item) => (
                      <button key={item.label} className="profile-item" onClick={() => item.label === "Informations Personnelles" ? openInfoDrawer() : item.label === "Sécurité & Biométrie" ? openSecurityModal() : item.label === "Historique des Reçus" ? openReceiptsModal() : item.label === "Support Client" ? openSupportModal() : item.label === "Conditions d'utilisation" ? openTermsModal() : item.label === "Confidentialité" ? openPrivacyModal() : showToast(item.label)}>
                ...
                <button className="profile-logout" onClick={() => setLogoutModalOpen(true)}>
                  Se déconnecter
                </button>
          ...
          <div className={`app-screen ${screen === "dashboard" ? "active" : ""}`}>
            <div className="content-scrollable dashboard-mode">
                <div className="top-header">
                  ...
                  <div className="top-actions" style={{ position: "relative" }}>
                    <button className="icon-pill" onClick={() => setNotificationsOpen((open) => !open)} aria-label="Ouvrir les notifications" style={{ border: notificationsOpen ? "1px solid rgba(59,130,246,.35)" : undefined, boxShadow: notificationsOpen ? "0 0 0 3px rgba(59,130,246,.12)" : undefined }}>
        ...
        {cameraScannerOpen && (
          <div className="card-modal-overlay" onClick={closeCameraScanner}>
            <div className="bc-modal" onClick={(event) => event.stopPropagation()}>
              <div className="bc-head">
                <div className="bc-head-left">
                ...
                <button className="bc-close" onClick={closeCameraScanner} aria-label="Fermer">&times;</button>
              </div>
              <div className="camera-modal-stage">
                <div className="camera-viewfinder" style={{ position: "relative", overflow: "hidden" }}>
                  ...
                  {scannerStatus === "error" && (
                    <div style={{ position: "absolute", inset: 0, zIndex: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, background: "rgba(5,11,26,0.9)", borderRadius: 32 }}>
                      ...
                      <button onClick={openCameraScanner} style={{ marginTop: 8, padding: "10px 24px", borderRadius: 14, border: "1px solid rgba(59,130,246,0.3)", background: "rgba(59,130,246,0.15)", color: "#60a5fa", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Réessayer</button>
                    </div>
                  )}
                </div>
                <div className="security-summary">
                  <div className="security-stat">
                    ...
                    <div className="security-stat-value" style={{ color: scannerStatus === "scanning" ? "var(--gold)" : scannerStatus === "found" ? "#4ade80" : "#fff" }}>{scannerStatus === "idle" ? "En attente" : scannerStatus === "scanning" ? "Scan actif" : scannerStatus === "found" ? "Détecté ✓" : "Erreur"}</div>
                  </div>
                  <div className="security-stat">
                    <div className="security-stat-kicker">Statut</div>
                    <div className="security-stat-value" style={{ color: scannerStatus === "scanning" ? "#4ade80" : "#fff" }}>{scannerStatus === "idle" ? "Caméra inactive" : scannerStatus === "scanning" ? "Caméra active" : scannerStatus === "found" ? "Connexion..." : "Indisponible"}</div>
                  </div>
                </div>
                <div className="camera-actions">
                  <button className="bc-btn-full" onClick={closeCameraScanner}>Fermer</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* ── Transaction validation confirmation sheet ── */}
        {transferConfirmOpen && (

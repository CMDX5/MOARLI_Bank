...
export async function POST(req: NextRequest) {
  ...
  const rl = rateLimit(`auth:send-reset:${clientId}`, { maxRequests: 3, windowSec: 120 });
  if (!rl.allowed) {
    return NextResponse.json({ error: "Trop de demandes. Réessayez dans 2 minutes." }, { status: 429 });
  }
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      ...
      to: email,
      subject: "Réinitialisation du mot de passe — Morali",
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px; background: #0a0e17; border-radius: 16px;">
          <div style="text-align: center; margin-bottom: 24px;">
            ...
            <p style="color: #64748b; font-size: 13px; margin-top: 4px;">Sécurité du compte</p>
          </div>
          <div style="background: rgba(239,68,68,0.06); border: 1px solid rgba(239,68,68,0.15); border-radius: 12px; padding: 20px; text-align: center; margin: 20px 0;">
            <p style="color: #94a3b8; font-size: 13px; margin: 0 0 8px;">Une demande de réinitialisation a été effectuée.</p>
            <p style="color: #94a3b8; font-size: 13px; margin: 0 0 16px;">Votre code de vérification :</p>
